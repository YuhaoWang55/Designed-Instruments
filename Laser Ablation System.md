...
6. Open 'Command' dialogue, choose and open the correspoding serial port.
  1. $$\textrm{\color{red}Upon opening the port, there will be a transient blink from the laser.}$$
  2. Make sure the 'Terminater by CR' is ticked.
...
8. Use eyepiece port to adjust the focus.
  1. $$\textrm{\color{red}Alarm: make sure the emission filter is in position before this step!}$$
  2. The laser spot should be located at the left field of view.
  3. A perfect focus is reached when a rectangle spot is observed.
  4. When a distorted laser spot, e.g., a tilted cone is observed, the obtical path is shifted. Call Wang Yuhao for re-adjusting the obtical path.
